...
# gameIDS
#gamelogs <- read_csv("data/teamgamelogs2015.csv")

gamelogs <- read_feather("data/teamgamelogs.feather")

glimpse(gamelogs)

gamelogs <-gamelogs %>% 
  mutate(team_id=as.integer(as.character(team_id)))
...
#bg_color = '#000004' # for court was black

bg_color = '#d6a262'###c2873e'
...
shotData <- eventReactive(input$go,{
...
## create an order of display based on points
  
  shotTot <- 
    shots %>% 
    group_by(game_id) %>% 
    summarize(points=sum(shot_made_numeric*shot_value)) %>% 
    ungroup() %>% 
    arrange(desc(points),desc(game_id)) %>% 
    mutate(game_id=as.character(game_id),pointOrder=row_number())
...
glimpse(shots) #nothing on season need to 
...
Regular Season {data-icon="fa-list"}
===================================== 
...
```{r inputs}
 includeMarkdown("shotChartInfo.md")
hr()
 inputPanel(
  selectInput(inputId = "player_name",
        ...
        sliderInput(inputId = "games",label="Games to Display", min=1,max=82, value=15),
  radioButtons(inputId="game_order", label="Arranged By", choices=c("Date","Points(excl FT"), inline=TRUE),
  actionButton("go","Click for Charts")
 )
...
```{r}
  ...
  gg <- base_court +
     geom_point(
       data = shots,
       aes(x = loc_x, y = loc_y,
           color = shot_made_flag, label=shot_distance),
       alpha = 0.8, size = 0.5
     ) +
     scale_color_manual("", values = c(made = "#FDE725", missed = "#1F9D89"))
  
   p <- ggplotly(gg, tooltip="label") %>% 
   layout(showlegend=FALSE, title=input$season,font=list(size=12))
    p
...
```{r}
 ...
 renderPlotly({
   ...
   shots <- shotData()$player %>% 
     filter(gameOrder==1)
    if (nrow(shots)==0) return()
   
   theTitle=paste0(shots$matchup[1]," ",shots$game_date[1])
    # shots$tooltip <- paste0("Distance: ",shots$shot_distance,"ft",
    #                         "<br>", shots$action_type)
   shots$Details <- paste0(shots$action_type," ",shots$shot_distance,"ft ",shots$shot_value,"pts")
  ...
  gg <- base_court +
     geom_point(
       data = shots,
       aes(x = loc_x, y = loc_y,
           color = shot_made_flag, label=Details),
       alpha = 1.0, size = 0.5
     ) +
     scale_color_manual("", values = c(made = "#FDE725", missed = "#1F9D89"))
